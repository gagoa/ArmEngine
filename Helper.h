...
/* Here is the instructor solution folder structure (if we make $(ProjectDir) a include directory, these paths are
 * valid. */
...
public:
    ...
    static SDL_Renderer* SDL_CreateRenderer(SDL_Window* window, int index, Uint32 flags) {
        ...
        if (renderer == nullptr) std::cerr << "Failed to create renderer : " << SDL_GetError() << std::endl;
    ...
    static void SDL_RenderPresent(SDL_Renderer* renderer) {
        if (renderer == nullptr) {
            std::cout << "ERROR : The renderer pointer passed to Helper::SDL_RenderPresent() is a nullptr."
                      << std::endl;
            exit(0);
        }

        if (input_status == NOT_INITIALIZED) {
            std::cout << "ERROR : Please do not attempt to render (Helper::SDL_RenderPresent()) before you've entered "
                         "the game loop (IE, begun calling Helper::SDL_PollEvent()."
                      << std::endl;
            exit(0);
        }
        ...
        if (RECORDING_MODE || _autograder_mode) {
            ...
            /* Read the current renderer's data and persist it as a .bmp file to disk (BMP format is fast-to-write
             * compared to PNG). */
            if (SDL_RenderReadPixels(renderer, NULL, SDL_PIXELFORMAT_RGB24, saving_surface->pixels,
                                     saving_surface->pitch)
                != 0) {
        ...
        if (!_autograder_mode) ::SDL_RenderPresent(renderer);   // The autograder doesn't need to render to window.
        SDL_Delay();
        frame_number++;
    }
    ...
    static void CheckForRenderLoggerInit() {
        ...
        if (render_logger_mode == RL_NOT_INITIALIZED) {
            if (IsLoggingMode()) {
                ...
                render_logging_file << "Study the following SDL_RenderCopyEx() calls to debug render-related issues."
                                    << std::endl;
                render_logging_file
                  << "Enable render logger mode on your computer by setting the RENDERLOGGER environmental variable."
                  << std::endl;
    ...
    static void SDL_RenderCopyEx(int actor_id, const std::string& actor_name, SDL_Renderer* renderer,
                                 SDL_Texture* texture, const SDL_FRect* srcrect, const SDL_FRect* dstrect,
                                 const float angle, const SDL_FPoint* center, const SDL_RendererFlip flip) {
        ...
        if (srcrect != nullptr) {
            srcrect_i = { static_cast<int>(srcrect->x), static_cast<int>(srcrect->y), static_cast<int>(srcrect->w),
                          static_cast<int>(srcrect->h) };
            srcrect_i_ptr = &srcrect_i;
        }

        if (dstrect != nullptr) {
            dstrect_i = { static_cast<int>(dstrect->x), static_cast<int>(dstrect->y), static_cast<int>(dstrect->w),
        ...
        if (render_logger_mode == RL_ENABLED) {
            ...
            if (dstrect != nullptr)
                render_logging_file << " dstrect " << dstrect->x << " " << dstrect->y << " " << dstrect->w << " "
                                    << dstrect->h;

            render_logging_file << " angle " << angle;
            if (center != nullptr) render_logging_file << " center " << center->x << " " << center->y;

            render_logging_file << " flip " << flip << " renderscale " << x_scale << " " << y_scale << std::endl;
        }
    }

    /* This encourages students to keep their data in float form as long as possible. We handle truncating to ints at
     * the very end for them, as necessary. */
    static void SDL_RenderCopy(SDL_Renderer* renderer, SDL_Texture* texture, const SDL_FRect* srcrect,
...
private:
    ...
    static void SDL_ConsiderInputFile() {
        ...
        if (RECORDING_MODE && input_status != INPUT_FILE_PRESENT && !_autograder_mode) {
            ...
            for (int i = 0; i < num_events; i++) {
                ...
                if (event_type == SDL_KEYUP || event_type == SDL_KEYDOWN || event_type == SDL_MOUSEMOTION
                    || event_type == SDL_MOUSEBUTTONDOWN || event_type == SDL_MOUSEBUTTONUP
    ...
    static void LoadSDLEventsFromInputFile() {
        if (IsAutograderMode()) _autograder_mode = true;
        ...
        while (std::getline(infile, line)) {
            ...
            while (std::getline(iss, eventStr, ';')) {
                ...
                event_type_string.erase(std::remove(event_type_string.begin(), event_type_string.end(), '\r'),
                                        event_type_string.end());

                if (event_type_string == "") continue;
                ...
                if (event_type == SDL_KEYUP || event_type == SDL_KEYDOWN) {
                    ...
                    if (mouse_button_index_str == "") continue;
                    ...
                    if (mouse_wheel_movement_str == "") continue;
...
public:
    ...
    inline float Sample() { return distribution(engine); }
};
