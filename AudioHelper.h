...
public:
    ...
    static inline int Mix_PlayChannel(int channel, Mix_Chunk* chunk, int loops) {
        std::cout << "(Mix_PlayChannel(" << channel << ",?," << loops << ") called on frame "
    ...
    static inline int Mix_Volume(int channel, int volume) {
        std::cout << "(Mix_Volume(" << channel << "," << volume << ") called on frame " << Helper::GetFrameNumber()
                  << ")" << std::endl;
    ...
    static inline void Mix_CloseAudio(void) {
        if (!IsAutograderMode()) ::Mix_CloseAudio();
    }

private:
    static inline Mix_Chunk autograder_dummy_sound;

    /* It's best for everyone if the autograder doesn't actually play any audio (it will still check that you try
     * though). */
